...
```{r}
cor_matrix <- soil %>%
  select(p_h, conductivity, moisture, temperature, temperature_annual_range, precipitation_of_wettest_month, precipitation_seasonality, precipitation_of_driest_month) %>%
  cor()
cor_long <- as.data.frame(as.table(cor_matrix))
colnames(cor_long) <- c("Climate_Soil_Var1", "Climate_Soil_Var2", "Correlation")
cor_long <- cor_long %>%
  mutate(elevation = mean(soil$elevation))
cor_long <- cor_long%>%
  mutate(elevation = runif(nrow(cor_long), min(soil$elevation), max(soil$elevation))) # Simulated elevation values
```

```{r}
# Heatmap with elevation as fill color
ggplot(cor_long, aes(x = Climate_Soil_Var1, y = Climate_Soil_Var2, fill = elevation)) +
  geom_tile() +
  scale_fill_gradient(low = "yellow", high = "red") +
  geom_text(aes(label = round(Correlation, 2)), size = 4, color = "black") +
  labs(title = "Climate-Soil Relationship Heatmap (Filled by Elevation)",
       x = "Climate & Soil Variables",
       y = "Climate & Soil Variables",
       fill = "Elevation") +
  theme_minimal() +
  ...
  tabPanel("Soil and Species",
   ...
   tabPanel("Species and Ecosystem Stability",
    fluidPage(
      fluidRow(
        column(6,
          plotOutput("plot_stability", height = 400, width = 400)
        ),
        column(6,
